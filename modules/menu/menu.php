...
class menu extends model {
	...
	private function initAdminMenu() {
		...
		$this->voices[__("Configuration")] = array(
			...
			__("DatetimePref") => $this->_registry->router->linkHref('datetimeSettings', 'manage'),
			__("Languages") => $this->_registry->router->linkHref('language', 'manage')
		); 
		$this->voices[__("Users")] = array(
			__("Users") => $this->_registry->router->linkHref('user', 'manage'),
			__("Groups") => $this->_registry->router->linkHref('group', 'manage'),
			__("Permissions") => $this->_registry->router->linkHref('privilege', 'manage')
		); 
